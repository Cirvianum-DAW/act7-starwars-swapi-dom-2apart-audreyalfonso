...
// Exemple d'inicialització de la llista de pel·lícules
...
async function _handleOnSelectMovieChanged(event) {
  ...
  // Esborrem l'anterior llistat de planetes
    const selector = document.querySelector('#select-homeworld');
    selector.innerHTML = '';

  // Pels planetes d'origen necessitem les dades de tots els personatges
...
function _populateHomeWorldSelector(homeworlds, selector) {
  // Implementem la lògica per "poblar" o injectar els planetes al desplegable "homeworld"
    const option = document.createElement('option');
    option.value = '';
